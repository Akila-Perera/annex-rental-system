    ...
    <div className="min-h-screen bg-[#0d1117] text-[#f0f4ff] overflow-x-hidden">
      ...
      <section className="min-h-screen relative overflow-hidden flex items-center justify-center px-8 pt-[100px] pb-[60px] hero-bg">
        ...
        <div className="max-w-[860px] text-center relative z-10">
          ...
          <div className="animate-fade-up delay-3 bg-[#161b25] border border-white/7 rounded-[20px] p-2.5 pl-6 flex flex-col md:flex-row items-stretch md:items-center gap-3 md:gap-0 shadow-[0_20px_60px_rgba(0,0,0,0.4),0_0_0_1px_rgba(45,126,247,0.1)] mb-5 search-focus transition-all duration-300">
            ...
            <button className="flex items-center justify-center gap-2 px-7 py-3.5 rounded-[14px] bg-blue-500 text-white text-[0.95rem] font-semibold cursor-pointer hover:bg-blue-400 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-blue-500/25 transition-all duration-300 flex-shrink-0 whitespace-nowrap md:ml-1">
              <span>🔍</span> Search Annex
            </button>
          </div>
        ...
        <div className="max-w-[1200px] mx-auto">
          <div className="flex justify-between items-end mb-9 flex-wrap gap-2.5">
            <div>
              ...
              <p className="text-[#8a96b0] text-[0.95rem]">Our most popular verified listings this month</p>
            </div>
            <a href="#all" className="text-blue-400 text-sm font-medium hover:text-blue-300 hover:tracking-wide transition-all duration-300">View All →</a>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {listings.map((item,i)=>(
              <div key={item.id} className="bg-[#1a2030] border border-white/7 rounded-[20px] overflow-hidden animate-slide-card hover:-translate-y-1.5 hover:border-blue-500/30 hover:shadow-[0_20px_60px_rgba(0,0,0,0.4),0_0_0_1px_rgba(45,126,247,0.15)] hover:bg-[#1e2638] transition-all duration-300"
                ...
                <div className="relative overflow-hidden h-[200px]">
                  ...
                  <span className={`absolute top-3 left-3 px-2.5 py-1 rounded-md text-[0.7rem] font-bold tracking-[0.05em] ${item.badgeClass}`}>{item.badge}</span>
                  <span className="absolute top-3 right-3 px-3 py-1.5 rounded-lg bg-black/70 backdrop-blur-sm text-white text-sm font-semibold border border-white/15">{item.price}</span>
                </div>
                <div className="p-5">
                  <div className="flex justify-between items-center mb-1.5">
                    <h3 className="font-display text-[1.05rem] font-bold text-[#f0f4ff]">{item.name}</h3>
                    <span className="text-amber-400 text-sm font-semibold">★ {item.rating}</span>
                  </div>
                  <p className="text-[#5a6478] text-[0.82rem] mb-4">📍 {item.distance}</p>
                  <div className="flex gap-2 mb-4 p-3 rounded-lg bg-white/3 border border-white/7">
                    {item.amenities.map(a=>(
                      <div key={a.label} className="flex-1 text-center">
                        ...
                        <span className="block text-[0.65rem] text-[#5a6478] font-semibold tracking-[0.05em]">{a.label}</span>
                      </div>
                    ))}
                  </div>
                  <button className="w-full py-3 bg-blue-500/10 border border-blue-500/30 text-blue-300 rounded-lg text-sm font-medium cursor-pointer hover:bg-blue-500 hover:text-white hover:border-blue-500 transition-all duration-300">View Details</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 px-8 text-center relative overflow-hidden bg-[#161b25] border-t border-b border-white/7">
        ...
        <div className="relative z-10">
          ...
          <div className="flex gap-3.5 justify-center flex-wrap">
            <a href="#search" className="px-9 py-3.5 rounded-[14px] bg-blue-500 text-white text-base font-semibold hover:bg-blue-400 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-blue-500/25 transition-all duration-300">Search Now</a>
      ...
      <footer className="bg-[#161b25] px-8 pt-[60px]">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1.5fr] gap-12 pb-12 border-b border-white/7">
          ...
          {[
            {title:'Explore',    links:[['#find','Find a Room'],['#maps','University Maps'],['#how','How it works'],['#reviews','Reviews']]},
